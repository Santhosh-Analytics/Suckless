...
void incrgaps(const Arg *arg) {
  gappx += arg->i; // Increment or decrement the gap size
  if (gappx < 0)   // Prevent negative gap sizes
    gappx = 0;
  arrange(NULL); // Recalculate the window arrangement
}
...
void hidewin(const Arg *arg) {
  ...
  // If there is no window selected (focused), return
  if (!win)
    return;

  // Withdraw the window from the display
  XWithdrawWindow(dpy, win,
                  0); // Withdraw the window from the first screen (index 0)
}
...
void restorewin(const Arg *arg) {
  ...
  // Map (show) the window again
  XMapWindow(dpy, win);
  XRaiseWindow(dpy, win); // Raise the window to the top
  arrange(selmon);        // Recalculate window arrangement
}
    ...
    for (keychord = oldoptions; keychord->n != 0 && currentkey < 5;
      ...
      if (keysym == keychord->keys[currentkey].keysym &&
        ...
        if (keychord->n == currentkey + 1) {
          ...
          numoption++;
          newoptions =
              (Keychord *)realloc(newoptions, numoption * sizeof(Keychord));
          memcpy((char *)newoptions + (numoption - 1) * sizeof(Keychord),
                 keychord, sizeof(Keychord));
        }
      }
    }
    currentkey++;
